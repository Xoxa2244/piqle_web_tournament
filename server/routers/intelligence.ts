...
function buildCohortFilterClause(f: CohortFilter): string {
    ...
    switch (f.field) {
      ...
      case 'sessionFormat':
        ...
        return `u.id::text IN (SELECT psb."userId"::text FROM play_session_bookings psb JOIN play_sessions ps ON ps.id = psb."sessionId" WHERE ps."clubId"::text = $1 AND ps.format = ${val} AND psb.status = 'CONFIRMED')`
      case 'dayOfWeek': {
        // Players who play on a specific day of week (Monday=1, Sunday=0)
        const dayMap: Record<string, number> = { Sunday: 0, Monday: 1, Tuesday: 2, Wednesday: 3, Thursday: 4, Friday: 5, Saturday: 6 }
        const dayNum = dayMap[String(f.value)] ?? 0
        return `u.id::text IN (SELECT psb."userId"::text FROM play_session_bookings psb JOIN play_sessions ps ON ps.id = psb."sessionId" WHERE ps."clubId"::text = $1 AND EXTRACT(DOW FROM ps.date) = ${dayNum} AND psb.status = 'CONFIRMED')`
      }
      case 'frequency': {
        ...
        return `u.id::text IN (SELECT psb."userId"::text FROM play_session_bookings psb JOIN play_sessions ps ON ps.id = psb."sessionId" WHERE ps."clubId"::text = $1 AND psb.status = 'CONFIRMED' AND psb."bookedAt" >= CURRENT_DATE - INTERVAL '30 days' GROUP BY psb."userId" HAVING COUNT(*) ${freqOp} ${freqVal})`
      }
      case 'recency': {
        ...
        return `u.id::text IN (SELECT psb."userId"::text FROM play_session_bookings psb JOIN play_sessions ps ON ps.id = psb."sessionId" WHERE ps."clubId"::text = $1 AND psb.status = 'CONFIRMED' GROUP BY psb."userId" HAVING MAX(ps.date) ${recOp} CURRENT_DATE - INTERVAL '${recVal} days')`
      }
      case 'userId':
      ...
      case 'riskLevel': {
        ...
        if (f.op === 'in' && Array.isArray(f.value)) {
          ...
          return `u.id::text IN (SELECT mhs.user_id::text FROM member_health_snapshots mhs WHERE mhs.club_id::text = $1 AND mhs.risk_level IN (${list}) AND mhs.date = (SELECT MAX(date) FROM member_health_snapshots mhs2 WHERE mhs2.club_id = mhs.club_id AND mhs2.user_id = mhs.user_id))`
        }
        const safeVal = String(f.value).replace(/'/g, "''")
        if (!['healthy', 'watch', 'at_risk', 'critical'].includes(safeVal)) return 'TRUE'
        return `u.id::text IN (SELECT mhs.user_id::text FROM member_health_snapshots mhs WHERE mhs.club_id::text = $1 AND mhs.risk_level = '${safeVal}' AND mhs.date = (SELECT MAX(date) FROM member_health_snapshots mhs2 WHERE mhs2.club_id = mhs.club_id AND mhs2.user_id = mhs.user_id))`
      }
      case 'joinedDaysAgo': {
...
const ACTIVE_MEMBER_JOIN = `
  JOIN (
    ...
    FROM play_session_bookings psb
    JOIN play_sessions ps ON ps.id = psb."sessionId"
    WHERE ps."clubId"::text = $1 AND psb.status = 'CONFIRMED'
...
async function fetchCohortBaseMembers(
  ...
  return prisma.$queryRawUnsafe(`
    ...
    ${latestHealthJoin}
    ${recentActivityJoin}
    WHERE cf.club_id::text = $1 AND ${where}
    ORDER BY u.name ASC
    ${limitClause}
  `, clubId)
}
  ...
  getMemberHealth: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        // ── Split-aggregate strategy for bookings (perf fix, 2026-05-28) ──
        //
        ...
        // counts and lifetime cancelled/no-show stats. SQL took
        // ~1.5–2s and the result blew up memory.
        //
        // After: a recent-window findMany + four typed-Prisma groupBy
        // aggregates, all running in parallel via Promise.all. The
        // per-booking scoring functions in lib/ai/member-health only
        // ever look at ≤60-day windows internally (scoreSessionDiversity
        // 30/60d, scorePatternBreak 14d, scoreCancelAcceleration 14/28d)
        // so the 90-day window is a strict superset. Lifetime counts
        // and the absolute last-confirmed timestamp come from the
        // groupBys — kept lifetime so lapsed members still get a
        // correct `daysSinceLastConfirmedBooking` even when their last
        // play was > 90 days ago.
        const now = new Date()
        ...
        // Five parallel typed-Prisma queries replace the old single
        // findMany that pulled every booking ever into memory. Why
        // typed and not raw SQL: the Sol2 prod schema diverges from
        // schema.prisma on UUID columns (clubId fields are TEXT in
        // DB despite @db.Uuid — see 4f45204f0). Raw SQL with `::uuid`
        // casts fails on prod with "operator does not exist: text =
        // uuid" — a regression that already burned us once this
        // session. Typed API sidesteps the trap.
        const [
          recentBookings,
          totalAgg,
          cancelledAgg,
          ...
          // B1 — total lifetime bookings per user (all statuses)
          ctx.prisma.playSessionBooking.groupBy({
            by: ['userId'],
            where: {
              userId: { in: userIds },
              playSession: { clubId: input.clubId },
          ...
          // B2 — lifetime cancelled count per user
          ctx.prisma.playSessionBooking.groupBy({
            by: ['userId'],
            where: {
              userId: { in: userIds },
          ...
          // B3 — lifetime no-show count per user
          ctx.prisma.playSessionBooking.groupBy({
            by: ['userId'],
            where: {
              userId: { in: userIds },
          ...
          // B4 — last CONFIRMED booking per user (kept absolute so
          // lapsed members still get correct daysSinceLastConfirmedBooking
          // even when their last play was > 90 days ago)
          ctx.prisma.playSessionBooking.groupBy({
            by: ['userId'],
            where: {
              userId: { in: userIds },
        ...
        log.info(
          `[Intelligence] getMemberHealth bookings: users=${userIds.length} recent90d=${recentBookings.length} totals=${totalAgg.length} cancelled=${cancelledAgg.length} noShow=${noShowAgg.length} lastConfirmed=${lastConfirmedAgg.length} preferences=${preferences.length}`
        )

        // Build lookup maps from the aggregate result sets
        const prefMap = new Map(preferences.map(p => [p.userId, p]))
        const totalMap = new Map(totalAgg.map(a => [a.userId, a._count._all]))
        ...
        const memberInputs = followers.map(f => {
          ...
          const totalBookings = totalMap.get(f.userId) ?? 0
          const cancelledCount = cancelledMap.get(f.userId) ?? 0
          const noShowCount = noShowMap.get(f.userId) ?? 0
          const lastConfirmedAt = lastConfirmedMap.get(f.userId) ?? null
          const daysSinceLast = lastConfirmedAt
            ? Math.floor((now.getTime() - new Date(lastConfirmedAt).getTime()) / 86400000)
            : null
          ...
          return {
            ...
            // bookingDates / bookingsWithSessions are last-90d slices.
            // All scoring functions in lib/ai/member-health.ts look at
            // ≤60d windows internally so this is semantically equivalent.
            // Lifetime totals (total/cancelled/noShow) come from the
            // groupBy aggregates above; revenue stays approximated from
            // the 90d window — scoring's value-tier classifier compares
            // members relative to each other within the same club, so a
            // consistent 90d window keeps the ordering correct.
...
Generate 3 campaign strategies with different goals and timings based on the data above.`,
  ...
  launchCampaign: protectedProcedure
    .input(z.object({
      ...
      // Recurring track. Required when format='recurring'. MVP cron
      // expressions are generated by the Wizard from a structured
      // frequency selector (daily / weekly+dow / monthly+dom). Custom
      // cron text input is a v2 feature — see R-track spec.
      // The runner uses a minimal in-tree matcher for the patterns the
      // Wizard generates; pre-validate format here so admins get an
      // error if a malformed expression sneaks through.
      ...
      recurringTimezone: z.string().max(64).optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      await requireClubAdmin(ctx.prisma, input.clubId, ctx.session.user.id)
      ...
      if (input.format === 'recurring') {
        if (!input.cronExpression || input.cronExpression.trim().length === 0) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Recurring campaigns require a schedule (cron expression).',
          })
        }
        // Cohort source is required; userIds inline lists make no sense
        // for recurring (the audience is re-evaluated each tick from
        // the cohort filters).
        if (!input.cohortId) {
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Recurring campaigns must target a saved cohort. Inline user lists are not supported (the cohort is re-evaluated each run).',
          })
        }
      }
      ...
      // For one_time + sequence we require a non-empty audience at launch.
      // Recurring is allowed to start with 0 — the cohort might fill up
      // before the first cron tick — but we still warn callers via the
      // returned recipientCount so the wizard can surface "0 right now".
      if (input.format !== 'recurring' && recipientIds.length === 0) {
