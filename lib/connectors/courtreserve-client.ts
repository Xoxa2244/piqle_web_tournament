...
export class CourtReserveClient {
  ...
  // read-only (we sync data in, we never push changes out). Earlier
  // speculative write helpers (reactivate / suspend family membership,
  // assign custom rating, create court block, plus a private writeRequest
  // POST/PUT helper) were removed: CR write-back isn't available to us, so
  // any "change it in CourtReserve" step is surfaced to the operator as
  // manual advice, never an API call.
}
