...
export function generateOutreachMessages(
  ...
  const { memberName, clubName, lowComponents, daysSinceLastActivity, preferredDays, suggestedSessionTitle } = input
  ...
  if (type === 'CHECK_IN') {
    return generateCheckinMessages(name, clubName, session, days, prefDay, primary)
  }
  return generateRetentionMessages(name, clubName, session, days, prefDay, primary)
}

// ── CHECK_IN messages (watch members, health 50-74) ──

function generateCheckinMessages(
  name: string, club: string, session: string, days: number, prefDay: string, primary: string,
): OutreachMessageVariant[] {
  ...
  if (prefDay) {
    variants.push({
      ...
      emailBody: `Hi ${name},\n\nWe noticed you missed your usual ${prefDay} session at ${club}. Everything OK?\n\nWe have "${session}" coming up and would love to see you there. Your spot is always open!\n\nSee you on the courts,\n${club} Team`,
      smsBody: `Hey ${name}! Missed you on ${prefDay} at ${club}. "${session}" is coming up — hope to see you there!`,
    })
  }
  ...
  variants.push({
    ...
    emailBody: `Hi ${name},\n\nWe noticed your visits to ${club} have slowed down a bit lately. No worries — we just want to make sure you know there are some great sessions coming up.\n\n"${session}" could be a perfect fit for you. Come join the fun!\n\nBest,\n${club} Team`,
    smsBody: `Hi ${name}! Your visits to ${club} have been a bit less frequent. "${session}" is coming up — great chance to get back in the groove!`,
  })
  ...
  variants.push({
    ...
    emailBody: `Hi ${name},\n\nIt's been ${days} days since your last game at ${club}. We bet you're itching to play!\n\n"${session}" is coming up and we'd love to see you back on the court. Don't let the paddle gather dust!\n\nCheers,\n${club} Team`,
    smsBody: `Hey ${name}! ${days} days without pickleball? "${session}" at ${club} is calling your name. See you there?`,
  })
...
function generateRetentionMessages(
  ...
  variants.push({
    ...
    emailBody: `Hi ${name},\n\nWe've noticed a significant drop in your visits to ${club}, and honestly — we miss having you around. You're an important part of our community.\n\nWe have "${session}" coming up and it would be great to see you there. We've saved a spot just for you.\n\nLet's get you back on the courts!\n${club} Team`,
    smsBody: `${name}, we really miss you at ${club}! "${session}" is coming up. We saved you a spot — hope you can make it.`,
  })
  ...
  variants.push({
    ...
    emailBody: `Hi ${name},\n\nIt's been ${days} days since your last session at ${club}. We know life gets busy, but we don't want you to lose your rhythm.\n\n"${session}" has limited spots left, and we've earmarked one for you. Would love to see you back!\n\nBest,\n${club} Team`,
    smsBody: `Hi ${name}! ${days}d since your last game. "${session}" has a spot for you at ${club}. Don't miss out!`,
  })
  ...
  if (prefDay) {
    variants.push({
      ...
      emailBody: `Hi ${name},\n\nYou used to be a regular on ${prefDay}s at ${club}, and we've noticed you've been away. We'd love to help you get back into your groove.\n\n"${session}" is a great way to restart. Your pickleball buddies are waiting!\n\nSee you soon,\n${club} Team`,
      smsBody: `${name}, missing your ${prefDay} sessions at ${club}! "${session}" is a great way to restart. See you there?`,
    })
  } else {
    variants.push({
      ...
      emailBody: `Hi ${name},\n\nOur community at ${club} has been growing, and we want to make sure you're still part of it. It's been a while since we've seen you on the courts.\n\n"${session}" is coming up — it's a great way to reconnect with the group!\n\nWarm regards,\n${club} Team`,
      smsBody: `${name}, the ${club} community misses you! "${session}" is coming up. Great chance to reconnect.`,
    })
  }
