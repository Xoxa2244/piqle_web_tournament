...
describe('A/B тестирование > Выбор лучшего варианта', () => {
  ...
  it('достаточно данных → best_performer (лучший по engagement)', async () => {
    ...
    // Count calls for checkin_pattern: opens=3, clicks=1, bounces=0, converted=0
    mockPrisma.aIRecommendationLog.count
      ...
      .mockResolvedValueOnce(0)  // pattern converted
      // Count calls for checkin_frequency: opens=8, clicks=5, bounces=0, converted=2
      .mockResolvedValueOnce(8)  // frequency opens
      .mockResolvedValueOnce(5)  // frequency clicks
      .mockResolvedValueOnce(0)  // frequency bounces
      .mockResolvedValueOnce(2)  // frequency converted

    // Mock Math.random to select champion (return < 0.80)
  ...
  it('20% шанс challenger (тестирует не лучший вариант)', async () => {
    // Initial groupBy returns data
    mockPrisma.aIRecommendationLog.groupBy
      .mockResolvedValueOnce([
        { variantId: 'checkin_pattern', _count: { id: 15 } },
    ...
    mockPrisma.aIRecommendationLog.count
      ...
      .mockResolvedValueOnce(0)  // pattern bounces
      .mockResolvedValueOnce(5)  // pattern converted
      .mockResolvedValueOnce(2)  // frequency opens
      .mockResolvedValueOnce(1)  // frequency clicks
      .mockResolvedValueOnce(0)  // frequency bounces
      .mockResolvedValueOnce(0)  // frequency converted

    // Force challenger selection (random >= 0.80)
    const originalRandom = Math.random
    Math.random = () => 0.95
    ...
    try {
      ...
      expect(result.reason).toBe('challenger')
