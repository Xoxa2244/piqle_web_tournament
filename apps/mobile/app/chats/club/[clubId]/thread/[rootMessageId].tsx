...
export default function ClubChatThreadScreen() {
  ...
  const params = useLocalSearchParams<{ clubId: string; rootMessageId: string; name?: string }>()
  ...
  const threadQuery = trpc.clubChat.listThread.useQuery(
    ...
    { enabled: Boolean(clubId) && Boolean(rootMessageId) && isAuthenticated, ...messageThreadRealtimeQueryOptions }
  )
  ...
  const sendMessage = trpc.clubChat.send.useMutation({
    ...
    onSuccess: (data: any, _vars, context: any) => {
      ...
      utils.clubChat.listThread.setData({ clubId, rootMessageId }, (current: any) => {
  ...
  const likeMessage = trpc.clubChat.likeMessage.useMutation({
    onMutate: async ({ messageId }: { messageId: string }) => {
      await utils.clubChat.listThread.cancel({ clubId, rootMessageId })
  ...
  if (!isAuthenticated) {
    return (
      <Screen title="Replies" subtitle="Sign in to access club messages.">
        <AuthRequiredCard title="Authentication required" body="Club chat is available only for signed in members." />
      </Screen>
    )
  }
    ...
    <PageLayout chatAmbient scroll={false} contentStyle={styles.screen} topBarTitle="Replies" topBarRightSlot={null}>
      {threadQuery.error ? (
        <SurfaceCard tone="soft">
          <Text style={styles.error}>{threadQuery.error.message}</Text>
        </SurfaceCard>
      ) : null}
      ...
      >
        <ChatThreadRoot
          ref={scrollRef}
          contentContainerStyle={[styles.scrollContent, messages.length === 0 && styles.messagesEmpty]}
        ...
        >
          {messages.length === 0 ? (
            ...
            <ChatThreadMessageList
              ...
              canDelete={(m) => Boolean((user?.id && m.userId === user?.id) || isAdmin) && !m.isDeleted}
