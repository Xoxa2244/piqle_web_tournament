...
import { mockGuestTrialBooking, mockMemberHealth, mockReferralSnapshot, mockSmartFirstSession, mockWinBackSnapshot } from '../_data/mock'
...
export default function MembersPage() {
  ...
  const iqMockMemberHealth = useMemo(() => mockMemberHealth(), [])
  if (brand.key === 'iqsport') return <MembersIQ memberHealthData={iqMockMemberHealth} memberGrowthData={memberGrowthData} smartFirstSessionData={mockSmartFirstSession as any} guestTrialBookingData={mockGuestTrialBooking as any} winBackSnapshot={mockWinBackSnapshot as any} referralSnapshot={mockReferralSnapshot as any} isLoading={false} sendOutreach={sendOutreach} sendReactivation={sendReactivation} clubId={clubId} reactivationCandidates={reactivationData?.candidates} aiProfiles={aiProfilesMap} onRegenerateProfiles={() => regenerateProfiles.mutate({ clubId })} />
