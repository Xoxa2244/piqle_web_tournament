...
export function MembersIQ({ memberHealthData, memberGrowthData, smartFirstSessionData, guestTrialBookingData, winBackSnapshot, referralSnapshot, isLoading: externalLoading, sendOutreach, clubId, reactivationCandidates, aiProfiles, onRegenerateProfiles, sendReactivation }: MembersIQProps = {}) {
  ...
  // Quick-send handleOutreach removed 2026-06-12 (restore from git with the buttons).
  ...
  return (
    ...
    >
      ...
      {/* 2026-05-28: Removed four canned insight blocks from this view
          per product request — Guest/Trial Booking, Smart First Session,
          Win-Back, and Referral Engine. The summary computations and
          launch-from-insights buttons (suggestedAgentLaunches above)
          remain so they can be re-surfaced via the Insights drawer or
          a new Insights tab if needed. Resurrect any of these from
          git history (was here pre-deletion, ~1765 lines). */}
...
{/* Quick-send Email/SMS removed 2026-06-12 per Solomon (выпилим на будущее) — restore from git history (handleOutreach) when the outreach UX returns */}
                </div>
              </Card>
            </motion.div>
          ))}
