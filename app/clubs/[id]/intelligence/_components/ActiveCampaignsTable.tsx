...
import { Mail, MessageSquare, Pause, Play, StopCircle, AlertTriangle, Layers, Repeat } from 'lucide-react'
                    ...
                    <tr key={campaign.id} style={{ borderTop: '1px solid var(--divider)' }}>
                      <td className="py-2.5 pr-3 font-semibold" style={{ color: 'var(--heading)' }}>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {campaign.sendFormat === 'sequence' && (
                            <span
                              className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] uppercase tracking-wider"
                              style={{ background: 'rgba(139,92,246,0.16)', color: '#A78BFA', fontWeight: 700 }}
                              title={`Sequence campaign — ${campaign.totalSteps ?? 1} step${(campaign.totalSteps ?? 1) === 1 ? '' : 's'}.`}
                            >
                              <Layers className="h-2.5 w-2.5" />
                              Sequence · {campaign.totalSteps ?? 1}
                            </span>
                          )}
                          {campaign.sendFormat === 'recurring' && (
                            <span
                              className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] uppercase tracking-wider"
                              style={{ background: 'rgba(6,182,212,0.18)', color: '#67E8F9', fontWeight: 700 }}
                              title={campaign.recurringDescription ?? 'Recurring campaign'}
                            >
                              <Repeat className="h-2.5 w-2.5" />
                              {campaign.recurringDescription ?? 'Recurring'}
                            </span>
                          )}
                        </div>
                      </td>
