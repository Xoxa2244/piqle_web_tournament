...
export default function EditClubPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.replace(`/auth/signin?callbackUrl=${encodeURIComponent(`/clubs/${clubId}/edit`)}`)
    }
  }, [status, router, clubId])
  ...
  const setupAutocomplete = useCallback(async () => {
    ...
    try {
      ...
      setAddressError(err instanceof Error ? err.message : 'Failed to load Google Places.')
    }
  }, [])
  ...
  const handleAddressBlur = async () => {
    ...
    try {
      ...
      geocoder.geocode({ address: value }, (results: any, geocodeStatus: any) => {
  ...
  if (isLoading) {
    return <div className="space-y-6 px-6 py-8 text-sm text-muted-foreground">Loading club…</div>
  }
  ...
  if (error || !club) {
    return (
      <div className="space-y-6 px-6 py-8">
        ...
        <Card>
          <CardContent className="py-8 text-sm text-muted-foreground">{error?.message || 'Club not found.'}</CardContent>
        </Card>
      </div>
    )
  }
  ...
  if (!club.isAdmin) {
    return (
      <div className="space-y-6 px-6 py-8">
        <Link href={`/clubs/${clubId}`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-gray-900">
          <ArrowLeft className="h-4 w-4" />
          Back to club
        </Link>
        <Card>
  ...
  return (
    <div className="space-y-6 px-6 py-8 max-w-2xl">
      ...
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold">Edit club</h1>
        <p className="text-sm text-muted-foreground">Update your club profile and booking details.</p>
      </div>
